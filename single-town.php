<?php
	...
	$title = get_field('name').'の街情報｜'.SITE_NAME;
	...
	if (have_posts()) {
		...
		// ▼カテゴリ、タグのターム取得
		$aryCatTag = getCategoryTag($post, 'town');
...
?>
					<p class="ft_size8"><?= $aryCatTag['cat'][0][1] ?>の街、<?= $ary['name'] ?>（アイスランド表記：<?= $ary['is_name'] ?>）の街情報です。</p>
					<?php get_template_part('content', 'cattag'); ?>
					<div class="mb20">
...
?>
						<div class="col-md-3 col-sm-4 col-xs-6 text-center col-list mb20">
							<a href="<?= $value['image_lg'] ?>"><img class="img-responsive center-block" src="<?= $value['image_thumb'] ?>" alt="<?= $ary['name'] ?>の画像"></a>
						</div>
<?php
						}
					}
